...
export class ListService {
  ...
  async create(createListDto: CreateListDto[], boardId: string) {
    ...
    const { data: cardsData, error: cardsError } =
      await this.supbaseService.client.from('card').upsert(cards).select();
    if (cardsError) {
  ...
  async findAllCardsInLists(boardId: string) {
    const { data: board, error: boardError } =
      await this.supbaseService.client
        .from('board')
        .select()
        .eq('id', boardId);
    if (board.length === 0) {
    ...
    const { data: lists, error } = await this.supbaseService.client
      .from('list')
      .select()
      .eq('boardId', boardId)
      .order('position');
  ...
  async updateLists(lists: UpdateListDto[], boardId: string) {
    ...
    const updateLists = lists.map((list, index) => {
      return this.supbaseService.client
        .from('list')
  ...
  async updateListCard(cardId: string, cardPosition: number, listId: string) {
    ...
    const { data: card, error: cardError } = await this.supbaseService.client
      .from('card')
      .select()
      .eq('id', cardId)
      .single();
  ...
  async getLaBelCards(cardId: string) {
    ...
    const promises = data.map(async (label) => {
      const { data: labelData, error } = await this.supbaseService.client
        .from('board_label')
        .select()
  ...
  async getAssignedUsers(cardId: string) {
    ...
    const promises = data.map(async (user) => {
      const { data: userData, error } = await this.supbaseService.client
        .from('user')
        .select()
        .eq('id', user.user_id)
        .single();
